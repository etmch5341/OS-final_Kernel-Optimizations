...
constexpr uint32_t PUNISHMENT = 1;
...
void handle_timer(RegisterState* regs) {
    ...
    // Debug::printf("Preempting task on core %d\n", SMP::me());
    //schedulers.mine().currTask->vRunTime = schedulers.mine().currTask->vRunTime + PUNISHMENT;
    schedulers.mine().replaceCurrent();
