...
PageDir fini_cr3() {
    ...
    vmm_off();
    unref_page(pd.ppn(), [](PageNum ppn, uint32_t refs) { return true; });
    return pd;
}
...
PageDir exchange_cr3(PageDir pd) {
    ...
    init_cr3(pd);
    unref_page(old_pd.ppn(), [](PageNum ppn, uint32_t refs) { return true; });
    return old_pd;
}
...
extern "C" void vmm_pageFault(VirtualAddress va, RegisterState* regs) {
    Debug::printf("Virtual Address: %x\n");
